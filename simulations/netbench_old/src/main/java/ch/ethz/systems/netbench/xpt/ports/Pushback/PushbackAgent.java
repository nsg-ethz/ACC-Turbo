...
public class PushbackAgent {
    ...
    public void identifyAggregate(double estimatedArrivalRate, double linkCapacity) {
        ...
        System.out.println("Identify aggregate: (estimatedArrivalRate: " + estimatedArrivalRate + " (bit/s), linkCapacity: " + linkCapacity + " (bit/s))");
        ...
        for (int i=0; i<=aggReturn.finalIndex; i++) {
            ...
            if (rateLimitSession != null) {
                System.out.println("Identified an aggregate which was already in the rate-limiting list: " + aggSpec.dstPrefix + ". Rate limited to: " + aggReturn.limit);
            ...
            if (numSessions >= PushbackConstants.MAX_SESSIONS) {
                ...
                if (rank >= PushbackConstants.MAX_SESSIONS) {
                    System.out.println("Identified aggregate, but it is not in the top " + PushbackConstants.MAX_SESSIONS + " responsibles: " + aggSpec);
                    continue;
                }
            }
            ...
            RateLimitSession rls = new RateLimitSession(aggSpec, estimate, true, initialLimit, PushbackConstants.RATE_LIMIT_TIME_DEFAULT, aggReturn.limit);
    ...
    void pushbackCancel(RateLimitSession rls) {
        System.out.println("Stopping rate-limiting for aggregate: " + rls.aggSpec);
