...
public class RateLimitSession {
    ...
    public double expiryTime;  // time when this rate-limiting should expire (seconds)
    ...
    public RateLimitSession(AggSpec aggSpec, double rateEstimate, boolean initial,
                                       double limit, double delay, double lowerBound){
        ...
        this.expiryTime = this.startTime + delay;  // In seconds
        this.refreshedTime = this.startTime;  // In seconds

        System.out.println("Created rate-limiting session. Aggregate: " + aggSpec.dstPrefix
