...
def analyze_file(file_name):
    ...
    for timestamp, buf in pcap:
        ...
        start_time = datetime.datetime(2018, 12, 1, 10, 00, 00, 000000)
        end_time = datetime.datetime(2018, 12, 1, 10, 1, 00, 000000)
        ...
        # We perform a zoom in the UDP ports 0-100 (both for src and dest)
        if udp.sport > 100 or udp.dport > 100:
            continue 
...
if __name__ == '__main__':
    ...
    file = open('1normal_plotting_zoom.dat', 'w+')    
    file.write("#,Throughput\n")


    for src in range(0,100):
        for dst in range (0,100):
            key = str(src) + "," + str(dst)
            if key in dict_throughput:
