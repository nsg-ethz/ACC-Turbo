...
public class SimulationLogger {
    ...
    // DDoS-AID Extension
    private static BufferedWriter writerInputThroughputMalicious;
    private static BufferedWriter writerInputThroughputBenign;
    private static boolean inputThroughputTrackingEnabled;

    private static BufferedWriter writerOutputThroughputMalicious;
    private static BufferedWriter writerOutputThroughputBenign;
    private static boolean outputThroughputTrackingEnabled;

    private static BufferedWriter writerPacketDropsMalicious;
    private static BufferedWriter writerPacketDropsBenign;
    private static boolean packetDropsTrackingEnabled;

    // ACC-R Extension
    private static BufferedWriter writerAggregateInputThroughput;
    private static boolean aggregateInputThroughputTrackingEnabled;
    private static BufferedWriter writerAggregateOutputThroughput;
    private static boolean aggregateOutputThroughputTrackingEnabled;
    private static BufferedWriter writerAggregateDrops;
    private static boolean aggregateDropsTrackingEnabled;
    ...
    public static void open(NBProperties tempRunConfiguration) {
        ...
        if (tempRunConfiguration != null) {
            ...
            // DDoS-AID: Enabling logs
            inputThroughputTrackingEnabled = tempRunConfiguration.getBooleanPropertyWithDefault("enable_input_throughput_tracking", false);
            outputThroughputTrackingEnabled = tempRunConfiguration.getBooleanPropertyWithDefault("enable_output_throughput_tracking", false);
            packetDropsTrackingEnabled = tempRunConfiguration.getBooleanPropertyWithDefault("enable_packet_drops_tracking", false);

            // ACC-R: Enabling logs
            aggregateInputThroughputTrackingEnabled = tempRunConfiguration.getBooleanPropertyWithDefault("enable_aggregate_input_throughput_tracking", false);
            aggregateOutputThroughputTrackingEnabled = tempRunConfiguration.getBooleanPropertyWithDefault("enable_aggregate_output_throughput_tracking", false);
            aggregateDropsTrackingEnabled = tempRunConfiguration.getBooleanPropertyWithDefault("enable_aggregate_drops_tracking", false);
        }
        ...
        try {
            ...
            // DDoS-AID log writers
            if(inputThroughputTrackingEnabled){
                writerInputThroughputMalicious = openWriter("input_throughput_malicious.csv.log");
                writerInputThroughputBenign = openWriter("input_throughput_benign.csv.log");
            }
            if(outputThroughputTrackingEnabled){
                writerOutputThroughputMalicious = openWriter("output_throughput_malicious.csv.log");
                writerOutputThroughputBenign = openWriter("output_throughput_benign.csv.log");
            }
            if(packetDropsTrackingEnabled){
                writerPacketDropsMalicious = openWriter("packet_drops_malicious.csv.log");
                writerPacketDropsBenign = openWriter("packet_drops_benign.csv.log");
            }

            // ACC-R log writers
            if (aggregateInputThroughputTrackingEnabled){
                writerAggregateInputThroughput = openWriter("aggregate_input_throughput.csv.log");
            }
            if (aggregateOutputThroughputTrackingEnabled){
                writerAggregateOutputThroughput = openWriter("aggregate_output_throughput.csv.log");
            }
            if (aggregateDropsTrackingEnabled){
                writerAggregateDrops = openWriter("aggregate_drops.csv.log");
            }
    ...
    public static void close() {
        ...
        try {
            ...
            // DDoS-AID: Close log files
            if(inputThroughputTrackingEnabled){
                writerInputThroughputMalicious.close();
                writerInputThroughputBenign.close();
            }
            if(outputThroughputTrackingEnabled){
                writerOutputThroughputMalicious.close();
                writerOutputThroughputBenign.close();
            }
            if(packetDropsTrackingEnabled){
                writerPacketDropsMalicious.close();
                writerPacketDropsBenign.close();
            }

            // ACC-R: Close log files
            if (aggregateInputThroughputTrackingEnabled){
                writerAggregateInputThroughput.close();
            }
            if (aggregateOutputThroughputTrackingEnabled){
                writerAggregateOutputThroughput.close();
            }
            if (aggregateDropsTrackingEnabled){
                writerAggregateDrops.close();
            }
    ...
    /* DDoS-AID Logging */
    public static void logInputThroughput(long currentTime, long packetSizeBit, boolean malicious) {
        try {
            if (malicious){
                writerInputThroughputMalicious.write(currentTime + "," + packetSizeBit + "\n");
            } else {
                writerInputThroughputBenign.write(currentTime + "," + packetSizeBit + "\n");
            }
        } catch (IOException e) {
            throw new LogFailureException(e);
        }
    }
    public static void logOutputThroughput(long currentTime, long packetSizeBit, boolean malicious) {
        try {
            if (malicious){
                writerOutputThroughputMalicious.write(currentTime + "," + packetSizeBit + "\n");
            } else {
                writerOutputThroughputBenign.write(currentTime + "," + packetSizeBit + "\n");
            }
        } catch (IOException e) {
            throw new LogFailureException(e);
        }
    }
    public static void logPacketDrops(long currentTime, long packetSizeBit, boolean malicious) {
        try {
            if (malicious){
                writerPacketDropsMalicious.write(currentTime + "," + packetSizeBit + "\n");
            } else {
                writerPacketDropsBenign.write(currentTime + "," + packetSizeBit + "\n");
            }
        } catch (IOException e) {
            throw new LogFailureException(e);
        }
    ...
    /* ACC-R: Logging */
    public static void logAggregateInputThroughput(long currentTime, long packetSizeBit, long flowId) {
        try {
            writerAggregateInputThroughput.write(currentTime + "," + packetSizeBit + "," + flowId + "\n");
        } catch (IOException e) {
            throw new LogFailureException(e);
        }
    }
    public static void logAggregateOutputThroughput(long currentTime, long packetSizeBit, long flowId) {
        try {
            writerAggregateOutputThroughput.write(currentTime + "," + packetSizeBit + "," + flowId + "\n");
        } catch (IOException e) {
            throw new LogFailureException(e);
        }
    }
    public static void logAggregateDrops(long currentTime, long packetSizeBit, long flowId) {
        try {
            writerAggregateDrops.write(currentTime + "," + packetSizeBit + "," + flowId + "\n");
        } catch (IOException e) {
            throw new LogFailureException(e);
        }
    }
    ...
    // DDoS-AID Extension
    public static boolean hasInputThroughputTrackingEnabled () { return inputThroughputTrackingEnabled; }
    public static boolean hasOutputThroughputTrackingEnabled () { return outputThroughputTrackingEnabled; }
    public static boolean hasPacketDropsTrackingEnabled () { return packetDropsTrackingEnabled; }

    // ACC-R Extension
    public static boolean hasAggregateInputThroughputTrackingEnabled () { return aggregateInputThroughputTrackingEnabled; }
    public static boolean hasAggregateOutputThroughputTrackingEnabled () { return aggregateOutputThroughputTrackingEnabled; }
    public static boolean hasAggregateDropsTrackingEnabled () { return aggregateDropsTrackingEnabled; }
