...
import sys
sys.path.append('/data/bf-sde-9.5.0/install/lib/python2.7/site-packages/tofino')
...
class CoreAPI:
    ...
    def ddosaid_get_and_clear_counter(self):
        entries = []
        t = self.tables["MyIngress.do_bytes_count"]
        for (d, k) in t.entry_get(self.dev_tgt):
            entries.append((k.to_dict(), d.to_dict()))

        for key_value in range(4):
            t.entry_mod(self.dev_tgt,
                [t.make_key(
                    [gc.KeyTuple("queue_id", key_value)])],
                [t.make_data(
                    [gc.DataTuple('$COUNTER_SPEC_BYTES', 0)],
                    'MyIngress.bytes_count')]
            )
        return entries