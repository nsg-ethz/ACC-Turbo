...
function launch_attack(queue, size, flows, id)
        ...
        local DST_IP            = "20.0.0.50" -- modified afterwards
        local SRC_PORT          = 111
        local DST_PORT          = 222

        local mempool = memory.createMemPool(function(buf)

                -- We compute the dst IP: Instead of a single dst IP, we target the whole prefix 20.0.0.*
                local firstBytes = "20.0.0."
                local lastByte = math.random(255)
                local dstIPString = firstBytes .. tostring(lastByte)
                local DST_IP_MODIFIFED = parseIPAddress(dstIPString)

                buf:getUdpPacket():fill{ 
                        ethSrc = queue, -- get the src mac from the device
                        ethDst = DST_MAC,
                        ip4Src = SRC_IP,
                        ip4Dst = DST_IP_MODIFIFED,
                        ip4TOS = 0,
                        ipv4Length = 0,
                        ip4TTL = 250,
                        -- ipProtocol
